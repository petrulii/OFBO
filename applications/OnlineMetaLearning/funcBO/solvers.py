...
class Solver:
	...
	def add_ridge(self, hessian):
		#self.reg = 0.01
		#sys.stderr.write(f"add_ridge called with reg: {self.reg})\n")
		hessian += self.reg*torch.eye(hessian.shape[0], dtype= hessian.dtype, device= hessian.device)
		return hessian

	def get_wandb_dict(self):
		"""
		Return a single dictionary suitable for wandb.log() from the data_logs list.
		"""
		result = {}
		for log in self.data_logs:
				iter_num = log.get('inner_iter', 0)
				for key, value in log.items():
						if key == 'inner_iter':
								result[key] = value
						else:
								result[f"{key}_{iter_num}"] = value
		return result
...
class ClosedFormSolver(Solver):
	...
	def make_linear_system(self, dual_model_inputs, outer_grad):
